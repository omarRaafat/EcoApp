...
listTable()
function listTable(){
    orderList = new List("orderList", options).on("updated", function (list) {
...
function refreshCallbacks() {
    Array.from(removeBtns).forEach(function (btn) {
        btn.addEventListener("click", function (e) {
            ...
            itemDataId = e.target.closest("tr").children[1].dataset.id;
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                console.log(itemId,isdeleteid,itemDataId)
                    document.getElementById("delete-record").addEventListener("click", function () {
                        orderList.remove("id", isElem.outerHTML);
                        deleteOrderAjax(itemDataId)
                        $('#trID'+itemDataId).remove();
                        listTable()
                        location.reload()
...
function deleteMultiple(){
    ...
    for (i = 0; i < items.length; i++) {
        if (items[i].checked == true) {
            ...
            var _id = trNode.querySelector("td").dataset.id;
            eleIds_array.push(id);
            ids_array.push(_id);
        }
    }
    if (typeof ids_array !== 'undefined' && ids_array.length > 0) {
        Swal.fire({
            ...
            if (result.value) {
                for (i = 0; i < eleIds_array.length; i++) {
                    orderList.remove("id", `<a href="apps-ecommerce-order-details.html" class="fw-medium link-primary">` + eleIds_array[i] +`</a>`);
                }
                ...
                deleteArrayOrderAjax(ids_array)
                listTable()
                location.reload()
                Swal.fire({
                    title: 'Deleted!',
...
function deleteOrderAjax(order_id)
{
    console.log(order_id)
    $.ajax({
        url: '/vendor/orders/'+order_id,
        type: 'DELETE',
...
function deleteArrayOrderAjax(order_ids)
{
    $.ajax({
        url: '/vendor/orders/multi-delete',
        type: 'POST',
        data:{ids:order_ids},
        success: function(result) {
            console.log(result)
