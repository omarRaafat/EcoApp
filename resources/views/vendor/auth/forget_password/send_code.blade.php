...
@section('title')
    @lang('translation.confirm_phone')
@endsection
@section('content')
                        ...
                        <div class="card mt-4">
                            ...
                            <div class="card-body p-4">
                                <div class="text-center mt-2">
                                    <h5 class="text-primary">@lang('translation.confirm_phone')</h5>
                                    <p class="text-muted">@lang('translation.confirm_phone')</p>
                                    @if(session()->has('error'))
                                    <div class="alert alert-danger alert-dismissible alert-outline fade show mb-xl-0" role="alert">
                                        <strong> @lang('translation.error') </strong> {{session()->get('error')}}
                                ...
                                <div class="alert alert-borderless alert-warning text-center mb-2 mx-2" role="alert">
                                    @lang('translation.we_have_sent_sms_to_your_phonw_with_code')
                                </div>
                                <div class="p-2">
                                    <form action="{{route('vendor.password.confirm-phone')}}" method="POST">
                                        @csrf
                                        <div class="mb-4">
                                            <label for="username" class="form-label">@lang('translation.code')</label>
                                            <input type="hidden" name="phone" value="{{session()->get('phone')}}">
                                            <input type="text" class="form-control @error('code') is-invalid @enderror" value="{{ old('code') }}" id="username" name="code" placeholder="@lang('translation.code_placeholder')" required>
                                            @error('code')
