...
{{--                     # {{ $order->vendor->tax_num }}--}}
            ...
            <h6>
                <span style="font-weight: normal; color: gray">
                    @if ($order->status === \App\Enums\OrderStatus::REFUND)
                        @lang("admin.transaction_invoice.refund_date"):
                    @else
            ...
            <h6>
                <span style="font-weight: normal; color: gray">
                    @lang('admin.shipping_type')
                </span>
                @if($order->orderVendorShippings)
                    @foreach($order->orderVendorShippings as $orderVendorShipping)
                        <span> {{$orderVendorShipping->shippingType->title}} </span>
                    @endforeach
                @endif
            </h6>
