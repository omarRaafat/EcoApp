        ...
        <tr>
            ...
            <td width="40%" style="text-align: left">
                <table style="direction: rtl; width: 100%">
                    ...
                    @if($order->transaction?->type == 'order')
                        <tr>
                            <td style="text-align: right;">@lang("admin.delivery.domestic-zones.deliver-fees")</td>
                            {{-- @if(auth()->user()->vendor_id) --}}
                                <td style="text-align: left;">{{ $order->orderVendorShippings()->where('vendor_id' , $order->vendor_id)->first()?->total_shipping_fees ?? 0}} @lang("translation.sar")</td>
                            {{-- @else
                                <td style="text-align: left;">{{ $order->orderVendorShippings()->where('vendor_id' , $order->vendor_id)->first()?->total_shipping_fees ?? 0}} @lang("translation.sar")</td>
                            @endif --}}
                    ...
                    @if($order->transaction?->type == 'order')
                        <tr class="border-top border-top-dashed fs-15">
                            ...
                            <th style="text-align: left;">{{ $order->total + $order->orderVendorShippings()->where('vendor_id' , $order->vendor_id)->first()?->total_shipping_fees}} @lang("translation.sar")</th>
                        </tr>
                    @else
                        <tr class="border-top border-top-dashed fs-15">
                            <td style="text-align: right;">@lang("admin.tax-invoices.orders.total-amount")</td>
