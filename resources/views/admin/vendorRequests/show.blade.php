                                ...
                                <tbody class="list form-check-all">
                                    @if ($vendorRequests->count() > 0)
                                        @foreach($vendorRequests as $vendorRequest)
                                            <tr>
                                                <td class="id">
                                                    <a href="{{ route("admin.wareHouseRequests.show-products", $vendorRequest->id) }}"class="fw-medium link-primary">
                                                        #{{$vendorRequest->id}} 
                                                    </a>
                                                </td>
                                                ...
                                                <td class="product_en">{{ $vendorRequest?->qnt }}</td>
                                                <td class="product_en">{{ $vendorRequest?->mnfg_date?->format("Y-m-d") }}</td>
                                                <td class="product_en">{{ $vendorRequest?->expire_date?->format("Y-m-d") }}</td>
                                                <td>
                                                    <ul class="list-inline hstack gap-2 mb-0">
                                                        <li class="list-inline-item" data-bs-toggle="tooltip"
                                                            data-bs-trigger="hover" data-bs-placement="top" title="@lang('admin.wareHouseRequests.show')">
                                                            <a href="{{ route("admin.wareHouseRequests.show-products", $vendorRequest->id) }}"
