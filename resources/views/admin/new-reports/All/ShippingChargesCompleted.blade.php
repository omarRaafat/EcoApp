...
<body>
    ...
    <div style="height: 995px;position: absolute;left: 20px;right: 20px;top: 0;">
        ...
        @php 
            $CartVendor_ids = \App\Models\CartVendor::where('shippingType','Aramex')->where('status',3)->pluck('id')->toArray();
            $orders = \App\Models\CartVendor::where('shippingType','Aramex')->where('status',3);
            if(request()->get('date') != null && request()->get('date') != ''){
            ...
            if(request()->get('completed_date') != null && request()->get('completed_date') != ''){
                $ids =  \App\Models\OrderStatusDate::whereIn('order_id',$CartVendor_ids)->where('isCartVendor',1)
                ->whereDate('created_at','>=',request()->get('completed_date'))
            ...
            if(request()->get('completed_date_to') != null && request()->get('completed_date_to') != ''){
                $ids =  \App\Models\OrderStatusDate::whereIn('order_id',$CartVendor_ids)->where('isCartVendor',1)
                ->whereDate('created_at','<=',request()->get('completed_date_to'))
                ->pluck('order_id')->toArray();
         ...
         <table  id="exampleee" class="display" style="width:100%"> 
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ date('Y-m-d, H:i', strtotime($completed_date)) }}</td>
                    <td> {{ $order->id  }} </td>
