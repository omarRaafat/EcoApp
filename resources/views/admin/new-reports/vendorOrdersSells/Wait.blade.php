        ...
        <div class="text-right">
            <form action=" " method="get">
                <div class="row">
                    <div class="col-md-2">
                        <select name="vendor_id" class="form-control" required>
                            ...
                            @foreach (\App\Models\Vendor::orderBy('shop_name','asc')->get() as $item)
                                <option value="{{$item->id}}" @if($item->id == request()->get('vendor_id')) selected @endif>{{$item->shop_name}}</option>
                            @endforeach
                        </select>
                    </div> 
                    <div class="col-md-3">
                    ...
                    <div class="col-md-1">
                        <button type="submit" class="btn btn-sm btn-success mt-1 ml-1"> <i class="fas fa-search"></i> </button>
                    </div>
                        <div class="col-md-3 text-left">
                            @if(!empty(request()->get('vendor_id')))
                            <a href=" ?action=printAll&vendor_id={{ request()->get('vendor_id') }}&page={{ request()->get('page') }}&search={{ request()->get('search') }}&date={{ request()->get('date') }}&date_to={{ request()->get('date_to') }}" class="btn btn-success mt-4" style="width: 100%">طباعة</a>
                            @endif
                    </div>
                </div>
            </form>
