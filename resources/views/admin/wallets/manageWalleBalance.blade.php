                    ...
                    <form action="{{ route("admin.wallets.increaseAndDecreaseAmount", $wallet->id) }}" method="post">
                        ...
                        <div class="row">
                            <div class="col-lg-3">
                                ...
                                <input type="text" name="amount" value="{{ old("amount") }}" class="form-control" placeholder="@lang("admin.customer_finances.wallets.transaction.amount")"/>
                                @error('amount')
                            ...
                            <div class="col-lg-3">
                                ...
                                <select class="select2 form-control" name="type" id="select2_type">
                                    ...
                                    @foreach ($statusOfWalletTransactionsHistory as $state)
                                        <option value="{{ $state["value"] }}" @selected((string)$state["value"] == old('type'))>
                                            {{ $state["name"] }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-3">
                                ...
                                <select class="select2 form-control" name="transaction_type" id="select2_type">
                                    ...
                                    @foreach (\App\Enums\WalletTransactionTypes::getTypesListWithClass() as $state)
                                        <option value="{{ $state["value"] }}" @selected((string)$state["value"] == old('transaction_type'))>
                                            {{ $state["name"] }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-3">
                                <div class="justify-content-end">
                                    ...
                                    <button type="submit" class="form-control btn btn-primary" id="add-btn">
                                        @lang("admin.customer_finances.wallets.transaction.save")
                                    </button>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-xxl-2 col-sm-6">
                                <div>
                                    <input name="created_at" type="date"
                                        value="{{ request()->has('created_at') && request()->get('created_at') != '' ? Carbon\Carbon::parse(request()->get('created_at'))->format("Y-m-d") : '' }}"
                            ...
                            <div class="col-xxl-2 col-sm-4">
                                <div>
                                    <select class="form-control" data-choices data-choices-search-false
                                            name="type" id="type">
                                        <option @selected(request()->get('type') == 'all') value="all" selected>@lang("admin.customer_finances.wallets.all")</option>
                                        <option @selected(request()->get('type') == '1') value="1">@lang("admin.customer_finances.wallets.transaction.add")</option>
                                        <option @selected(request()->get('type') == '0') value="0">@lang("admin.customer_finances.wallets.transaction.sub")</option>
                                    </select>
                                </div>
                            </div>
