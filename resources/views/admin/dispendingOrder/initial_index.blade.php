...
@section('title')
    @lang('admin.dispensingOrder.initialDispensingOrder_title')
@endsection
@section('css')
            ...
            <div class="card" id="wallets">
                ...
                <div class="card-header border-0">
                    <div class="d-flex align-items-center">
                        <h5 class="card-title mb-0 flex-grow-1">@lang('admin.dispensingOrder.initialDispensingOrderFilter')</h5>
                    </div>
                </div>
                <div class="card-body border border-dashed border-end-0 border-start-0">
                    <form method="get" action="{{ route("admin.initialDispensingOrder.index")  }}">
                        <div class="row g-3">
                            ...
                            <div class="col-xxl-2 col-sm-4">
                                <div>
                                    <a href="{{route('admin.initialDispensingOrder.index')}}" type="reset" class="btn btn-secondary w-100"> <i class=" ri-loader-3-line me-1 align-bottom"></i>
                ...
                <div class="card-body border border-dashed border-end-0 border-start-0">
                    <form action="{{ route('admin.initialDispensingOrder.change')  }}" method="post">
                        @csrf
                        ...
                        <div class="table-responsive table-card mb-1">
                            <table class="table table-nowrap align-middle" id="walletsTable">
                                ...
                                <tr class="text-uppercase">
                                    <th>@lang('admin.dispensingOrder.id')</th>
...
{{--                                    <th>@lang('translation.actions')</th>--}}
                                ...
                                @foreach($dispensingOrders as $initial)
                                    <tr>
                                        <td class="id">
                                            #{{$initial->id}}
                                        </td>
                                        <td class="admin_id">{{ $initial->vendor?->getTranslation('name' ,'ar') }}</td>
                                        {{-- <td class="admin_id">{{ $wallet->vendor?->getTranslation('name' ,'en') }}</td> --}}
                                        <td class="customer_id">{{ $initial->vendor?->owner?->name }}</td>
                                        <td class="amount">{{ $initial->total_amount  . ' ' . __('translation.sar') }}</td>
...
{{--                                        </td>--}}
                        ...
                        <div class="d-flex justify-content-end">
                            <div class="pagination-wrap hstack gap-2">
                                {{ $dispensingOrders->appends(request()->query())->links("pagination::bootstrap-4") }}
                            </div>
                        </div>
                    </div>
                </div>
