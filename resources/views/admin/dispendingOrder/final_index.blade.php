...
@section('title')
    @lang('admin.dispensingOrder.finalDispensingOrder_title')
@endsection
@section('css')
            ...
            <div class="card" id="wallets">
                ...
                <div class="card-header border-0">
                    <div class="d-flex align-items-center">
                        <h5 class="card-title mb-0 flex-grow-1">@lang('admin.dispensingOrder.finalDispensingOrderFilter')</h5>
                    </div>
                </div>
                <div class="card-body border border-dashed border-end-0 border-start-0">
                    <form method="get" action="{{ route("admin.finalDispensingOrder.index")  }}">
                        <div class="row g-3">
                            ...
                            <div class="col-xxl-2 col-sm-4">
                                <div>
                                    <a href="{{route('admin.finalDispensingOrder.index')}}" type="reset" class="btn btn-secondary w-100"> <i class=" ri-loader-3-line me-1 align-bottom"></i>
                ...
                <div class="card-body border border-dashed border-end-0 border-start-0">
                    <form action="{{ route('admin.finalDispensingOrder.change')  }}" method="post">
                        @csrf
...
{{--                                    <th>@lang('translation.actions')</th>--}}
                                ...
                                @foreach($dispensingOrders as $final)
                                    <tr>
                                        <td class="id">
                                            #{{$final->id}}
                                        </td>
                                        <td class="admin_id">{{ $final->vendor?->getTranslation('name' ,'ar') }}</td>
                                        {{-- <td class="admin_id">{{ $wallet->vendor?->getTranslation('name' ,'en') }}</td> --}}
                                        <td class="customer_id">{{ $final->vendor?->owner?->name }}</td>
                                        <td class="amount">{{ $final->total_amount  . ' ' . __('translation.sar') }}</td>
