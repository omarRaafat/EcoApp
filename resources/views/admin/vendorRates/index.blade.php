                    ...
                    <form action="{{ route("admin.vendorRates.index") }}">
                        <div class="row g-3">
                            ...
                            <div class="col-xxl-2 col-sm-6">
                                <div>
                                    <select name="relation" class="form-control" data-choices data-choices-search-false  id="idStatus">
                                        ...
                                        <option @selected(request()->get('relation') == 'client') value="client">@lang("admin.vendorRates.user_id")</option> <!-- new relation filter -->
                                    </select>
                                </div>
                            </div>
                            <!--end col-->
                            <div class="col-xxl-2 col-sm-4">
                                <div>
                                    <select name="admin_approved" class="form-control" data-choices data-choices-search-false  id="idStatus">
                                        <option @selected(request()->get('admin_approved') == '') value="" selected>@lang("admin.vendorRates.filter_is_active")</option>4
                            ...
                            <div class="col-xxl-1 col-sm-1">
                                <div>
                                    <a href="{{ route('admin.vendorRates.index' , [
                                        'action' => 'exportExcel' ,
                                    ...
                                    @if ($vendorRates->count() > 0)
                                        @foreach($vendorRates as $rate)
                                            <tr>
                                                ...
                                                <td class="user_id">{{ $rate?->client->name  ?? NULL }}</td>
                                                ...
                                                <td>
                                                    <ul class="list-inline hstack gap-2 mb-0">
                                                        ...
                                                        <li>
                                                            <form action="{{ route('admin.vendorRates.update', $rate->id) }}" method="post" class="form-steps" autocomplete="on">
                                                                @csrf
                                                                @method('PUT')
...
@section('script')
    ...
    <script>
        function updateChecks(action){
            ...
            $.ajax({
                url: "{{ route('admin.vendorRates.update-checks') }}",
                type: 'POST',
                data: { 
                    "checks": selectedIds,
                    "action": action,
