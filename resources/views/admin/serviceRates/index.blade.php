                            ...
                            <div class="col-xxl-2 col-sm-6">
                                <div>
                                    <select name="relation" class="form-control" data-choices data-choices-search-false  id="idStatus">
                                        <option @selected(request()->get('relation') == 'all') value="all" selected>@lang("admin.serviceRates.all_serviceRates")</option>
                                        <option @selected(request()->get('relation') == 'service') value="service">@lang("admin.serviceRates.service_id")</option>
                                        <option @selected(request()->get('relation') == 'user') value="user">@lang("admin.serviceRates.user_id")</option>
                                    </select>
                                </div>
                            </div>
                            <!--end col-->
                            <div class="col-xxl-2 col-sm-2">
                                <div>
                                    <select name="admin_approved" class="form-control" data-choices data-choices-search-false  id="idStatus">
                                        <option @selected(request()->get('admin_approved') == '') value="" selected>@lang("admin.serviceRates.filter_is_active")</option>
                            ...
                            <div class="col-xxl-1 col-sm-4">
                                <div>
                                    <button type="submit" class="btn btn-secondary w-100" onclick="SearchData();"><i
                                            ...
                                            @lang("admin.serviceRates.filter")
                                    </button>
                                </div>
                            </div>
                            <div class="col-xxl-1 col-sm-1">
                                <div>
                                    <a href="{{ route('admin.serviceRates.index' , [
                                        'action' => 'exportExcel' ,
                ...
                <div class="card-body pt-0">
                    <div>
                        <div class="table-responsive table-card mb-1">
                            <table class="table table-nowrap align-middle" id="serviceRatesTable">
                                <thead class="text-muted table-light">
                                <tr class="text-uppercase">
                                    <th>@lang('admin.serviceRates.id')</th>
                                    <th>@lang('admin.serviceRates.rate')</th>
                                    <th>@lang('admin.serviceRates.user_id')</th>
                                    <th>@lang('admin.serviceRates.service_id')</th>
                                    <th style="width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap">@lang('admin.serviceRates.admin_id')</th>
                                    <th style="width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap">@lang('admin.serviceRates.comment')</th>
                                    <th>@lang('admin.serviceRates.admin_approved')</th>
                                    <th>@lang('admin.serviceRates.reporting')</th>
                                ...
                                <tbody class="list form-check-all">
                                    @if ($serviceRates->count() > 0)
                                        @foreach($serviceRates as $rate)
                                            <tr>
                                                <td class="id">
                                                    <a href="{{ route("admin.serviceRates.show", $rate->id) }}"class="fw-medium link-primary">
                                                        #{{$rate->id}}
                                                    </a>
                                                </td>
                                                ...
                                                <td class="service_id">{{ $rate->service?->name ?? null }}</td>
                                                <td class="admin_id">{{ $rate->admin->name ?? trans("admin.serviceRates.not_found") }}</td>
                                                <td class="comment"> <small>{{ $rate->comment ?? trans("admin.serviceRates.not_found") }}</small> </td>
                                                ...
                                                <td class="reporting">{{ !empty($rate->reporting) ? trans("admin.serviceRates.yes") : trans("admin.serviceRates.no") }}</td>
                                                <td>
                                                    <ul class="list-inline hstack gap-2 mb-0">
                                                        <li class="list-inline-item" data-bs-toggle="tooltip"
                                                            data-bs-trigger="hover" data-bs-placement="top" title="@lang('admin.serviceRates.show')">
                                                            <a href="{{ route("admin.serviceRates.show", $rate->id) }}"
                                                        ...
                                                        <li>
                                                            <form action="{{ route('admin.serviceRates.update', $rate->id) }}" method="post" class="form-steps" autocomplete="on">
                                                                @csrf
                                                                @method('PUT')
                                                    ...
                                                    <div class="modal-content">
                                                        <div class="modal-body p-5 text-center">
                                                            ...
                                                            <div class="mt-4 text-center">
                                                                <h4>@lang('admin.serviceRates.delete_modal.title')</h4>
                                                                <p class="text-muted fs-15 mb-4">@lang('admin.serviceRates.delete_modal.description')</p>
                                                                <div class="hstack gap-2 justify-content-center remove">
                                                                    ...
                                                                    <form action="{{ route("admin.serviceRates.destroy", $rate->id) }}" method="post">
                                                                        @csrf
                                                                        @method("DELETE")
                                                                        <button type="submit" class="btn btn-primary" id="delete-record">
                                                                            @lang('admin.serviceRates.delete')
                                                                        </button>
                                                                    </form>
                                                                </div>
                                                            </div>
                                        ...
                                        <tr>
                                            <td colspan = "9">
                                                <div class="text-center">
                                                    @lang('admin.serviceRates.not_found')
                                                </div>
                                            </td>
                                        </tr>
                                    @endif
                    ...
                    <!-- End Delete Modal -->
                        <div class="noresult" style="display: none">
                            <div class="text-center">
                                    ...
                                    <h5 class="mt-2">@lang('admin.serviceRates.no_result_found')</h5>
                                </div>
                            </div>
                        </div>
                        <div class="d-flex justify-content-end">
                            <div class="pagination-wrap hstack gap-2">
                                {{ $serviceRates->appends(request()->query())->links("pagination::bootstrap-4") }}
                            </div>
                        </div>
                    </div>
                </div>
...
@section('script')
    ...
    <script>
        function updateChecks(action){
            ...
            $.ajax({
                url: "{{ route('admin.serviceRates.update-checks') }}",
                type: 'POST',
                data: {
                    "checks": selectedIds,
                    "action": action,
