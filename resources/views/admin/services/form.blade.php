<div class="row">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    {{ Form::select('category_id', ['' => 'اختار القسم'] + $main_categories, $row->category_id ?? old('category_id'), ['class' => 'js-example-basic-single form-select', 'id' => 'category']) }}
                    @error('category_id')
                ...
                <div class="mb-3">
                    <label for="choices-publish-status-input" class="form-label">@lang('admin.services.vendor')<span class="required"
                            style="color: red!important">*</span></label>
                    <select @if (!isset($row) || $row->vendor->is_international != 1) onchange="vendorChange(this)" @endif class="form-select"
                        name="vendor_id" id="main-choices-input-vendor_id">
                        <option value="">@lang('admin.services.vendor')</option>
                        @foreach ($vendors as $vendor)
                            <option data-international="{{ $vendor->is_international }}"
                                @if ((isset($row) && $row->vendor_id == $vendor->id) || old('vendor_id') == $vendor->id) selected @endif value="{{ $vendor->id }}">
                                {{ $vendor->name }}</option>
                        @endforeach
                    </select>
                    @error('vendor_id')
                                ...
                                <div class="avatar-title bg-light rounded">
                                    @if (isset($row))
                                        @if ($row->square_image_temp && $row->square_image_temp != $row->square_image)
                                            <img src="{{ $row->square_image_temp }}" id="service-img"
                                                class="avatar-md h-auto" />
                                        @else
                                            <img src="{{ $row->square_image }}" id="service-img"
                                                class="avatar-md h-auto" />
                                        @endif
                                    @endif
                                </div>
    ...
    <div class="col-lg-4">
        <div class="card">
            ...
            <div class="card-body">
                <div class="mb-3">
                    <label for="choices-publish-visibility-input" class="form-label">@lang('translation.is_active') (الإدارة)
                    </label>
                ...
                <div class="mb-3">
                    <label for="choices-publish-visibility-input" class="form-label">@lang('translation.is_active')
                        (البائع)</label>
    ...
    <script>
        $(document).ready(function() {
            ...
            function populateFields(categoryId, oldFields, existingFields) {
                if (categoryId) {
                    $.ajax({
                        url: '/admin/categories/' + categoryId + '/fields',
                        method: 'GET',
