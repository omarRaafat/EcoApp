...
use App\Repositories\Admin\PreharvestCategoryRepository;
use Illuminate\Contracts\Pagination\LengthAwarePaginator;

class PreharvestCategoryService
{
    public function __construct(
        public PreharvestCategoryRepository $repository,
    ...
    public function getAllMainCategoriesWithPagination(Request $request, int $perPage = 10, string $orderBy = "ASC") : LengthAwarePaginator
    {
        ...
        if($request->has("search")) {
            $categories->where('name->ar', 'LIKE', "%{$request->input('search')}%");
        }
    ...
    public function getCategoryUsingID(int $id) : PreharvestCategory
    {
    ...
    public function createCategory(Request $request) : array
    {
        ...
        if(!empty($category)) {
            ...
            $this->logger->InLog([
                ...
                'action' => "CreatePreharvestCategory",
                'model_type' => "\App\Models\PreharvestCategory",
    ...
    public function updateCategory(int $category_id, Request $request) : array
    {
        ...
        $this->logger->InLog([
            'user_id' => auth()->user()->id,
            'action' => "UpdatePreharvestCategory",
            'model_type' => "\App\Models\PreharvestCategory",
    ...
    public function deleteCategory(int $category_id) : array
    {
        ...
        if($isDeleted == true) {
            $this->logger->InLog([
                'user_id' => auth()->user()->id,
                'action' => "DeletePreharvestCategory",
                'model_type' => "\App\Models\PreharvestCategory",
                'model_id' => $category->id,
                'object_before' => $oldCategoryObject,
                'object_after' => $category
    ...
    private function _createImage(PreharvestCategory $category, Request $request) : void
    {
    ...
    private function _updateImage(PreharvestCategory $category, Request $request) : void
    {
    ...
    private function _deleteOldCategoryImage(PreharvestCategory $category) : void
    {
