...
class CreateShipment
{
    ...
    {
        if(OrderShip::where('gateway_order_id',$order->id)->first()){
        ...
        if ($responseee->successful()) {
            ...
            if (isset($responseee->json()['ProcessedPickup']['ProcessedShipments'][0]['ID']) && !empty($responseee->json()['ProcessedPickup']['ProcessedShipments'][0]['ID'])) {
                ...
                resolve(ApiRequestAction::class)->handle([
                    'name' => 'AramexPrintLabelAfterPickup',
                    'model_name' => 'Order',
                    'model_id' => $order->id,
                    'client_id' => $order->transaction->customer_id,
                    'url' => $url,
    ...
    public function preparePickupData(Order $order , Warehouse $warehouse): array
    {
        try {
            ...
            /* 
            $englishAddressOrigin = $warehouse->getTranslation('address','en');
            $englishAddress = json_decode("\"$englishAddressOrigin\"", true, 512, JSON_UNESCAPED_UNICODE);
            $englishAddress = $this->removeUnicodeEscapeSequences($englishAddress);

            try {
                $englishAddressArry = explode(",",str_replace(array('،',',,','&#x060c;'),',',$englishAddress));
                $englishAddress = (count($englishAddressArry)>=2) ? ($englishAddressArry[count($englishAddressArry)-2].', '.$englishAddressArry[count($englishAddressArry)-1]) : $englishAddress;
            } catch (\Throwable $th) {
            report($th);
            }*/
                    ...
                    "Shipments" => [
                        [
                            ...
                            "Consignee"=> [
                                ...
                                "AccountNumber"=> config("shipping.aramex.AccountNumber"),
                                "PartyAddress"=> [
                                    "Line1"=>  $order->transaction->city->getTranslation('name','en'),
                                ...
                                "Contact" =>  [
                                    ...
                                    "PhoneNumber1" => $order->transaction->client->phone,
                                    "PhoneNumber1Ext" => "",
                                    "PhoneNumber2" => "",
                                    "PhoneNumber2Ext" => "",
                                    "FaxNumber" => "",
            ...
            OrderShipFail::create([
                'order_id'=> $order->id,
                'orderVendorShipping_id'=> $this->orderVendorShipping->id,
                'shipping'=> 'aramex',
                'req' =>  "Error in preparePickupData()",
                'res' =>  $e->getMessage() . $e->getFile() . $e->getLine(),
            ]);
            return [];
        }
    }
