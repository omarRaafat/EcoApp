...
class OrderRefund {
    ...
    private string $url;
    private const EVENT_NAME = "Order_Refund";
    ...
    public function __invoke()
    {
        ...
        Log::channel("webengage-events")->info("ORDER_Refund_EVENT: start", ['request-body' => $data]);

        try {
            $response = Http::withToken(config("webengage.api_key"))
            ->post($this->url, $data)
            ->json();
            Log::channel("webengage-events")->info("ORDER_Refund_EVENT: end", ['response-body' => $response]);
        } catch (RequestException | ConnectionException $e) {
            Log::channel("webengage-events")->info("ORDER_Refund_EVENT: end-provider-exception", [
                'exception-message' => $e->getMessage(),
                'exception-trace' => $e->getTrace()
            ]);
        } catch (Exception | Error $e) {
            Log::channel("webengage-events")->info("ORDER_Refund_EVENT: end-internal-exception", [
                'exception-message' => $e->getMessage(),
                'exception-trace' => $e->getTrace()
            ]);
        }
