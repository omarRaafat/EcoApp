...
use App\Http\Requests\Admin\CreateCountryPriceRequest;
use App\Http\Requests\Admin\UpdateCountryPriceRequest;
use App\Models\Country;
use App\Services\Admin\ProductCountryPricesService;
...
class ProductCountryPricesController extends Controller
{
    ...
    public function index(Request $request, $productId)
    {
        ...
        $breadcrumbParentUrl = route('admin.products.edit',$productId);
        return view("admin.countries_prices.index", compact('countriesPrices', 'breadcrumbParent', 'breadcrumbParentUrl'));
    }
    ...
    {
        $availableCountry = Country::whereDoesntHave('countryPrices',function($q) use($id){
            $q->where('product_id',$id);
        })->select('id', 'name')->where('is_national', 0)->active()->get();
        $breadcrumbParent = 'admin.products.edit';
        $breadcrumbParentUrl = route('admin.products.edit',$id);
        return view("admin.countries_prices.create", compact('availableCountry', 'breadcrumbParent', 'breadcrumbParentUrl'));
    }
    ...
    public function store(CreateCountryPriceRequest $request)
    {
        ...
        if($result["success"] == TRUE)
        {
            ...
            return redirect()->route('admin.products.edit',$result['product_id']);
        }
    ...
    public function edit(int $id)
    {
        ...
        $breadcrumbParentUrl = route('admin.products.prices.list',$countryPrice->product_id);

        return view("admin.countries_prices.edit", compact('countryPrice', 'availableCountry', 'breadcrumbParent', 'breadcrumbParentUrl'));
    }
    ...
    public function update(UpdateCountryPriceRequest $request, int $id)
    {
        ...
        {
            Alert::success($result["title"], $result["body"]);
            return redirect()->route('admin.products.prices.list', ['id' => $result['product_id']]);
        }
        Alert::error($result["title"], $result["body"]);
        return redirect()->back();
    }
